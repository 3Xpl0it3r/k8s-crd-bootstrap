...
import (
	...
	croperator "github.com/l0calh0st/loki-operator/pkg/operator"
	"time"
)
...
func NewController(kubeClientSet kubeclientset.Interface) crcontroller.Controller {
	c := &controller{}
    return c
}

func (c *controller) Run(ctx context.Context) error {
    //todo write you custom code here
	return nil
}
...
// OnAdd is called when an $(fn_word_all_to_lower ${CRKind}) object is added.
func(c *controller)OnAdd(object interface{}){
    // call hooks 
...
// OnUpdate is called when an $(fn_word_all_to_lower ${CRKind}) object is update.
...
// OnDelete is called when an $(fn_word_all_to_lower ${CRKind}) object is delete
func(c *controller)OnDelete(object interface{}){
    // call hooks 
	for _,h := range c.GetHooks(){
		h.OnDelete(object)
	}
